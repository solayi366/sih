...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM tab_activotec WHERE id_activo = p_id_activo AND activo = TRUE
    ) THEN
    ...
    RETURN QUERY
    SELECT
        a.id_activo, a.serial, a.codigo_qr, a.hostname, a.referencia,
        a.mac_activo, a.ip_equipo, a.estado,
        a.password_activo,
        t.nom_tipo, m.nom_marca, mo.nom_modelo,
        e.nom_emple, e.cod_nom, ar.nom_area,
        a.id_padre_activo,
        tp.nom_tipo,
        ap.codigo_qr
    ...
    WHERE a.id_activo = p_id_activo AND a.activo = TRUE;
END;
$$ LANGUAGE plpgsql;