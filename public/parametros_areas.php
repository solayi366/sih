...
<body class="bg-slate-50 antialiased font-sans h-screen flex overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth w-full">
            <div class="max-w-[1200px] mx-auto">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                    <div>
                        <h1 class="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight ">Unidades de Negocio</h1>
                        <p class="text-slate-500 font-medium mt-1">Estructura organizacional del hospital.</p>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-200 text-xs font-bold text-slate-500  tracking-wider">
                                ...
                                <th class="px-6 py-4">Nombre de la Unidad</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-sm">
                            ...
                            <tr class="hover:bg-brand-50/20 transition-colors">
                                <td class="px-6 py-4 font-mono text-[10px] text-brand-600">#<?= str_pad($a['r_id'], 3, '0', STR_PAD_LEFT) ?></td>
                                <td class="px-6 py-4 text-slate-700 font-bold "><?= $a['r_nombre'] ?></td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button onclick="abrirEdicion(<?= $a['r_id'] ?>, '<?= $a['r_nombre'] ?>')" class="text-slate-400 hover:text-brand-600 transition-colors"><i class="fas fa-edit"></i></button>
                                        <button onclick="Alerts.confirmDelete('../controllers/parametrosController.php?ent=area&action=delete&id=<?= $a['r_id'] ?>')" class="text-slate-400 hover:text-rose-600 transition-colors"><i class="fas fa-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
    ...
    <div id="modalCrear" class="modal-overlay hidden fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="modal-container bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div class="bg-slate-900 px-6 py-4 flex items-center justify-between">
                <h3 class="text-white text-xs font-black  tracking-widest">Nueva Unidad</h3>
            ...
            <form action="../controllers/parametrosController.php?ent=area&action=create" method="POST">
                <div class="p-8"><input type="text" name="nom_area" placeholder="Nombre del Área..." required class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-brand-600 outline-none"></div>
    ...
    <div id="modalEdicion" class="modal-overlay hidden fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="modal-container bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div class="bg-slate-900 px-6 py-4 flex items-center justify-between">
                <h3 class="text-white text-xs font-black  tracking-widest">Actualizar Área</h3>
            ...
            <form action="../controllers/parametrosController.php?ent=area&action=update" method="POST">
                ...
                <div class="p-8"><input type="text" name="nom_area" id="edit_nombre" required class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-brand-600 outline-none"></div>
                <div class="bg-slate-50 px-6 py-4 flex gap-3">
