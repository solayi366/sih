        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-8 w-full">
            <div class="max-w-3xl mx-auto">
                ...
                <div class="flex items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 class="text-xl md:text-2xl font-extrabold text-slate-900 tracking-tight">Marcas</h1>
                        <p class="text-slate-500 text-sm mt-0.5">Fabricantes por categoría de equipo.</p>
                    </div>
                    <button onclick="abrirModal('modalCrear')"
                        ...
                        <span class="hidden sm:inline">Añadir Marca</span>
                        ...
                        <div class="flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="text-[10px] font-mono text-brand-600 hidden md:inline">#<?= str_pad($m['r_id'],3,'0',STR_PAD_LEFT) ?></span>
                                    <span class="text-[10px] font-black text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded"><?= htmlspecialchars($m['r_tipo']) ?></span>
                                </div>
                            </div>
                ...
                <div class="p-5 space-y-4">
                    <div>
                        ...
                        <select name="id_tipoequi" required class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-brand-600 outline-none">
                            ...
                            <option value="<?= $t['r_id'] ?>"><?= htmlspecialchars($t['r_nombre']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nombre de la Marca</label>
                        <input type="text" name="nom_marca" placeholder="Ej: Dell, HP, Lenovo..." required
        ...
        <div class="modal-container bg-white w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden">
            ...
            <form action="../controllers/parametrosController.php?ent=marca&action=update" method="POST">
                ...
                <div class="p-5">
                    ...
                    <input type="text" name="nom_marca" id="edit_nombre" required
                           class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-semibold focus:border-brand-600 outline-none transition-all">
                </div>
                <div class="bg-slate-50 px-5 py-4 flex gap-3">
