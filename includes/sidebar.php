...
<div id="mobile-overlay" onclick="toggleMobileMenu()" class="fixed inset-0 bg-black/50 z-30 hidden md:hidden transition-opacity"></div>

<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 flex flex-col h-full shadow-xl transform -translate-x-full md:translate-x-0 md:static transition-transform duration-300">
    <button onclick="toggleSidebarPC()" class="hidden md:flex absolute -right-3 top-20 bg-white border border-slate-200 text-slate-400 hover:text-brand-600 rounded-full w-6 h-6 items-center justify-center shadow-sm z-50 hover:scale-110 transition-transform">
    ...
    <div class="h-16 md:h-20 flex items-center justify-center border-b border-slate-100 shrink-0">
        <a href="dashboard.php" class="flex items-center gap-3 px-4">
            <div class="p-1.5 bg-brand-50 rounded-lg shrink-0"><img src="../assets/logo.png" alt="Logo" class="h-8 w-8 object-contain"></div>
            <span class="logo-text font-black text-xl tracking-tighter uppercase text-slate-900">SIH<span class="text-brand-600">QR</span></span>
        </a>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto no-scrollbar py-4 space-y-1 px-3">
        ...
        <a href="activos.php" class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all <?php echo isActive('activos.php', $current_page); ?>">
            <i class="fas fa-layer-group nav-icon w-6 text-center"></i><span class="sidebar-text ml-3">Inventario</span>
        </a>
        <a href="dashboard.php" class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all <?php echo isActive('dashboard.php', $current_page); ?>">
            <i class="fas fa-chart-pie nav-icon w-6 text-center"></i><span class="sidebar-text ml-3">Métricas</span>
        </a>
        ...
        <a href="crear_activo.php" class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all <?php echo isActive('crear_activo.php', $current_page); ?>">
            <i class="fas fa-plus-circle nav-icon w-6 text-center"></i><span class="sidebar-text ml-3">Nuevo Activo</span>
        </a>

        <div class="space-y-1">
            <button onclick="toggleSubmenu('submenu-params')" class="w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">
                <div class="flex items-center">
                    <i class="fas fa-sliders-h nav-icon w-6 text-center"></i><span class="sidebar-text ml-3">Configuración</span>
                </div>
                <i id="arrow-params" class="fas fa-chevron-down text-[10px] transition-transform <?php echo $is_param_page ? 'rotate-180' : ''; ?>"></i>
            </button>
            <div id="submenu-params" class="<?php echo $is_param_page ? '' : 'hidden'; ?> pl-10 space-y-1">
                <a href="parametros_tipos.php" class="block py-2 text-xs font-bold <?php echo ($current_page == 'parametros_tipos.php') ? 'text-brand-600' : 'text-slate-400 hover:text-brand-600'; ?>">Tipos</a>
    ...
    <div class="p-4 border-t border-slate-100 bg-slate-50/50 shrink-0">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-full bg-brand-600 text-white flex items-center justify-center font-bold text-sm shadow-md shrink-0"><?php echo $inicial_usuario; ?></div>
            <div class="sidebar-text user-info flex-1 min-w-0">
                ...
                <a href="../controllers/logout.php" class="text-[10px] text-red-500 hover:text-red-700 font-bold flex items-center gap-1 mt-0.5"><i class="fas fa-sign-out-alt"></i> Cerrar Sesión</a>
            </div>
        </div>
    </div>
</aside>
...
function toggleSubmenu(id) {
    ...
    const arrow = document.getElementById('arrow-params');
